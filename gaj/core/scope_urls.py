...
背景（2026-09-16 实测）：BOSS 列表 API 的 ``resCount`` 是虚高且不随筛选变化的
营销数字 —— ``AI测试@杭州`` 无论基线还是加筛选都报 ``resCount=450/hasMore=true``。
但**深翻页有效**：同一口径连翻 3 页各 15 条、两两零重叠（45 条去重仍未触底）；
**筛选参数也实测生效**：``&experience=105`` 的页面 15/15 全为「3-5年」，与基线口径
仅 3/15 重叠。所以扩池有两条路：先深翻页（同口径、零额外筛选成本），需要分层切面
（经验/薪资/学历/规模/融资）时用本模块生成筛选口径。
